...
async def scheduled_twitter_check():
    ...
    for query in queries:
        ...
        for tweet in tweets:
            ...
            async with async_session() as session:
                exists = await session.execute(select(News).where(News.url == f"https://twitter.com/i/web/status/{tweet['id']}"))
                if exists.scalar_one_or_none():
                    continue
            news_article = {
...
def setup_schedulers():
    # Обновляем пороги категорий раз в день
    scheduler.add_job(
        trigger_detector.update_thresholds,
...
async def cleanup_old_cache():
    # Можно удалять старые ключи Redis
    pass
